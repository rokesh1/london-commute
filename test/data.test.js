...
test("history excludes missing, unknown, expired and future samples from its denominator", () => {
  ...
  const sample = (at, state) => ({
    at,
    lines: state ? { central: state } : {},
  });
  const stats = summarize(
    [
      sample("2026-09-20T11:00:00Z", "good"),
      sample("2026-09-20T10:00:00Z", "disrupted"),
      sample("2026-09-20T09:00:00Z", "unknown"),
      sample("2026-09-20T08:00:00Z"),
      sample("2026-09-10T08:00:00Z", "good"),
      sample("2026-09-21T08:00:00Z", "good"),
      sample("invalid", "good"),
    ],
    "central",
    now,
...
test("filters combine saved lines, disruptions and case-insensitive search", () => {
  const lines = [
    { id: "central", name: "Central", state: "good" },
    { id: "district", name: "District", state: "disrupted" },
  ];
  assert.equal(
    filterLines(lines, { mode: "saved", saved: [], search: "" }).length,
    0,
  );
  assert.equal(
    filterLines(lines, { mode: "issues", saved: [], search: "" })[0].id,
    "district",
  );
  assert.equal(
    filterLines(lines, { mode: "all", saved: [], search: " CENT " })[0].id,
    "central",
  );
});
