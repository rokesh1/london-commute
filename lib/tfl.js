...
export function classify(statuses) {
  ...
  if (statuses.some((s) => [1, 2, 3, 4, 5, 6, 7, 16, 20].includes(s.code)))
    return "disrupted";
...
export function normalize(raw, fetchedAt = new Date().toISOString()) {
  if (!Array.isArray(raw) || raw.length === 0)
    throw new Error("Empty or invalid TfL response");
  const lines = raw.map((line) => {
    if (
      typeof line.id !== "string" ||
      typeof line.name !== "string" ||
      !Array.isArray(line.lineStatuses)
    ) {
