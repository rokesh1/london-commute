...
let saved = [];
try {
  const stored = JSON.parse(localStorage.getItem("commute-lines") || "[]");
  if (Array.isArray(stored))
    saved = stored.filter((id) => Object.hasOwn(COLORS, id));
} catch {
...
function renderCommute() {
  ...
  $("#commute-lines").innerHTML = chosen.length
    ? chosen
        .map(
          (line) =>
            `<span class="saved-chip"><i style="background:${COLORS[line.id] || "#777"}"></i>${escape(line.name)}</span>`,
        )
        .join("")
    : "Save your lines using the + button below.";
  const affected = chosen.filter((line) => line.state !== "good");
  $("#commute-status").textContent = chosen.length
    ? affected.length
      ? `${affected.length} of your ${chosen.length} saved lines have a notice. Check details below.`
      : "Your saved lines are reporting good service."
    : "Your selection stays on this device. No account needed.";
}

function renderLines() {
  const open = new Set(
    [...document.querySelectorAll(".line-card details[open]")].map(
      (el) => el.dataset.id,
    ),
  );
  const visible = filterLines(lines, {
    mode: filter,
    search: $("#search").value,
    saved,
  });
  $("#lines").innerHTML = visible.length
    ? visible
        ...
        .join("")
    : `<p class="empty">${filter === "saved" && !saved.length ? "No saved lines yet. Choose All lines, then use + to save your commute." : "No lines match this view. Try another filter or search."}</p>`;
  renderCommute();
}
...
function updateAge() {
  ...
  $("#updated").textContent =
    `${old ? "Last received" : "Updated"} ${time(fetchedAt)} London`;
  $("#live-label").textContent = old ? "Older data" : "Live feed";
  if (old)
    $("#commute-status").textContent =
      "Status is more than three minutes old. Refresh before travelling.";
}
...
async function refresh() {
  ...
  try {
    ...
    if (
      ...
      throw new Error(data.error || "Unexpected response from TfL.");
    lines = data.lines;
    ...
    $("#network-caption").textContent =
      lines.length === good
        ? "Good service across all monitored lines."
        : `${lines.length - good} lines with delays, closures or other notices.`;
    $("#network-stripes").innerHTML = lines
      .map((l) => `<span class="${escape(l.state)}"></span>`)
      .join("");
    const selected = $("#history-line").value;
    $("#history-line").innerHTML = lines
      .map((l) => `<option value="${escape(l.id)}">${escape(l.name)}</option>`)
      .join("");
    $("#history-line").value = lines.some((l) => l.id === selected)
      ? selected
      : lines[0].id;
    renderLines();
    renderHistory();
    ...
    $("#error").hidden = false;
    $("#error").textContent =
      `${error.name === "TimeoutError" ? "TfL took too long to respond." : "Live status could not be refreshed."} ${fetchedAt ? "Showing the last received status; check its timestamp." : "Please try Refresh or check TfL directly."}`;
    if (!fetchedAt) {
      $("#lines").innerHTML =
        '<p class="empty">Live status is unavailable. <a href="https://tfl.gov.uk/tube-dlr-overground/status/">Check TfL service status ↗</a></p>';
      $("#network-caption").textContent =
        "We couldn’t reach TfL. Please try again shortly.";
    }
...
function renderHistory() {
  ...
  $("#history-line-name").textContent =
    name + (["dlr", "elizabeth"].includes(id) ? "" : " line");
  ...
  const displayedSamples = samples.filter(
    (s) =>
      ...
      londonDate(s.at) >= londonDate(firstDay),
  );
  const summary = summarize(displayedSamples, id);
  $("#history-score").innerHTML =
    `${summary.percent === null ? "—" : summary.percent + "%"}<small>good-service samples</small>`;
  $("#history-detail").textContent = summary.known
    ? `${summary.good} of ${summary.known} known-status observations reported good service. ${summary.total - summary.known ? `${summary.total - summary.known} unknown observations excluded. ` : ""}This measures snapshots, not journey punctuality.`
    : "Observations are just getting started. Check back after the collector has run; no history is invented.";
  const timestamps = samples
    .map((s) => Date.parse(s.at))
    .filter(Number.isFinite);
  const latest = timestamps.length ? Math.max(...timestamps) : null;
  $("#history-freshness").textContent =
    (latest ? `Latest sample: ${dateTime(latest)} London. ` : "") +
    (scheduled
      ? latest && Date.now() - latest > 90 * 60000
        ? "Collection is delayed; gaps are not filled."
        : "Scheduled approximately every 30 minutes."
      : "Automatic collection is awaiting setup.");
  ...
  $("#chart").innerHTML = days
    .map((day) => {
      ...
      const daily = samples.filter(
        (s) => Number.isFinite(Date.parse(s.at)) && londonDate(s.at) === key,
      );
...
async function loadHistory() {
  try {
    ...
    historyFailed = true;
    $("#chart").innerHTML =
      '<p class="empty">History is temporarily unavailable. Live status is separate and can still be refreshed.</p>';
    $("#history-score").innerHTML = "—<small>history unavailable</small>";
    $("#history-freshness").textContent = "";
    $("#history-detail").textContent =
      "We couldn’t load the observation history. Try Refresh to load it again.";
  }
}
...
$("#lines").addEventListener("click", (event) => {
  ...
  try {
    localStorage.setItem("commute-lines", JSON.stringify(saved));
  } catch {
    storageFailed = true;
  }
  renderLines();
  ...
  if (storageFailed)
    $("#commute-status").textContent +=
      " Your browser could not save this selection for next time.";
});
